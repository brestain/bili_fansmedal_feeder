...
脚本会自动：
- 读取 `users.yaml` 中的账号信息
- 获取所有粉丝牌列表
- 生成 `fansmedal_weight.yaml` 文件（如果不存在）
- 更新已存在的 `fansmedal_weight.yaml`（保留自定义权重）

#### 6. 配置粉丝牌权重（可选）

打开 `fansmedal_weight.yaml`，修改各个直播间的权重：
...
**应用场景**：首次使用或新增粉丝牌后，需要生成或更新 `fansmedal_weight.yaml`。
(不更新或者不配置此文件也会正常去打卡, 只是权重默认使用100)  
**操作步骤**：

1. **确保已配置 users.yaml**：
   - `users.yaml` 中必须已填写正确的 `access_key`
...
3. **等待脚本执行**：
   - 脚本会自动读取 `users.yaml` 中的账号
   - 获取所有粉丝牌列表
   - 生成或更新 `fansmedal_weight.yaml`
...
1. **打开 fansmedal_weight.yaml**：
   ```powershell
   # Windows
   notepad fansmedal_weight.yaml
...
```
2025-01-09 20:38:23 B站粉丝牌助手 启动守护模式，任务将持续运行。
